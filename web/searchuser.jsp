<%-- 
    Document   : searchwork
    Created on : 7 Nov, 2012, 12:18:21 PM
    Author     : varun
--%>
    ...
    <body>
        ...
        <%
            ...
            while(results.next()){
                ...
                PreparedStatement pstmt=con.prepareStatement("select * from users_following where user_id_follower=? and user_id_followed=?");
        ...
        <form id="link" method="post" action="FollowHandle">
            <a href="profile.jsp?userid=<%=temp_userid%>" id="follower_name"><%=temp_firstname%> <%=temp_lastname%></a>
            <%if(!username.equals(temp_userid)){ %>
            <input name="user_or_work" value="user" type="hidden"/><input name="userid" value="<%=temp_userid%>" type="hidden"/><input name="action" value="<%=button_value%>" type="hidden"/><input name="asked_page" value="searchuser.jsp?query=<%=query%>" type="hidden"/><input id="button" type="submit" value="<%=button_value%>"/>
            <% } %>
        </form>
        <%
            }
