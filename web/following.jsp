<%-- 
    Document   : following
    Created on : 5 Nov, 2012, 1:22:33 AM
    Author     : varun
--%>
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Following | Wordsmith</title>
    ...
    <body>
        ...
        <div class="bg">
        	...
        	<br/><br/>
                            <div id="wrapper"><h1>Following</h1></div>
                            <section>
        <%
            PreparedStatement prepStmt2 = con.prepareStatement("select user_id_followed from users_following where user_id_follower=?");
            prepStmt2.setString(1,userid);
            ResultSet following=prepStmt2.executeQuery();
            while(following.next()){
                String following_id=following.getString(1);
                if(!following_id.equals(user_loggedin)){
                    PreparedStatement prepStmt3 = con.prepareStatement("select first_name,last_name from user where user_id=?");
                    prepStmt3.setString(1, following_id);
                    ResultSet followerinfo=prepStmt3.executeQuery();
                    String following_fn="",following_ln="";
                    if(followerinfo.next()){
                        following_fn=followerinfo.getString(1);
                        following_ln=followerinfo.getString(2);
                    }
                    ...
                    getfollow.setString(2, following_id);
        ...
        %>
        <form id="link" method="post" action="FollowHandle"><a href="profile.jsp?userid=<%=following_id%>" id="follower_name"><%=following_fn%> <%=following_ln%></a><input name="user_or_work" value="user" type="hidden"/><input name="userid" value="<%=following_id%>" type="hidden"/><input name="action" value="<%=button_value%>" type="hidden"/><input name="asked_page" value="following.jsp?userid=<%=userid%>" type="hidden"/><input id="button" type="submit" value="<%=button_value%>"/></form>
        <%
                }
            }
            con.close();
